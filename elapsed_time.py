...
class ElapsedTimeJob:
    ...
    def __init__(self, edge, chart_yr, d_dir, intro=''):
        ...
        self.__output_file = Path(str(d_dir.project_folder())+'/'+self.__edge_name+'.csv')
    ...
    def execute_callback(self, result):
        print(f'-     {self.__intro} {self.__edge_name} {"SUCCESSFUL" if isinstance(result[1], pd.core.frame.DataFrame) else "FAILED"}', flush=True)
