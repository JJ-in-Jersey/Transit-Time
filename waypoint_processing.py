...
def waypoint_processing(route, cy, job_manager):
    ...
    for wp in filter(lambda wp: isinstance(wp, TideStationWP)):
    ...
    for wp in filter(lambda wp: isinstance(wp, InterpolatedDataWP)):
    ...
    for wp in filter(lambda wp: isinstance(wp, InterpolatedWP)):
