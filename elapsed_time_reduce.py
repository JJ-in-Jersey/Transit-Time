...
from project_globals import mins_secs, file_exists, boat_speeds
...
def elapsed_time_reduce(folder, route):
    ...
    if file_exists(elapsed_times_file):
        ...
        rw.write_df(et_reduce_df, elapsed_times_file)
        print(f':     elapsed time reduce ({mins_secs(perf_counter() - init_time)} minutes)', flush=True)
    ...
    for speed in boat_speeds:
        ...
        if file_exists(speed_file):
            ...
            speed_df = rw.read_df(speed_file)
        else:
            ...
            rw.write_df(speed_df, speed_file)
