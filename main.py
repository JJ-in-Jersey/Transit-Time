...
if __name__ == '__main__':
    ...
    # for waypoint in route._path._velocity_waypoints: mpm.job_queue.put(VelocityJob(mpm, waypoint))
    # mpm.job_queue.join()
    # for waypoint in route._path._velocity_waypoints: waypoint._velo_array(mpm.result_lookup[id(wapoint)])
    vj = VelocityJob(mpm, route._velocity_waypoints[0])
    vj.execute()
